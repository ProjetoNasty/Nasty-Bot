...
module.exports = {
    name: 'addcargo',
    description: "Adicionar cargo em um membro",
    run: async (client, message, args) => {
        ...
        if (!membro) {
            ...
            let noPerm = new Discord.EmbedBuilder()
                .setDescription(`${message.author}, utilize ${prefixoNB}addcargo (@cargo/id) (@usuario/id)`)
                .setColor(`${colorNB}`)
        ...
        if (highest.comparePositionTo(role1) <= 0) {
            ...
            let noPerm = new Discord.EmbedBuilder()
                .setDescription(`${message.author}, você não pode adicionar um cargo acima de seu!`)
                .setColor(`${colorNB}`)
        ...
        const embed1 = new Discord.EmbedBuilder()
            .setDescription(`O cargo ${role} foi adicionado a ${membro}`)
            .setColor(`${colorNB}`)

        membro.roles.add(role1);
